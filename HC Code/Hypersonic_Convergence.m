...
for a = 1: 1: length(Wpay)
    ...
    for b = 1: 1: length(Ncrew)

        % Iterate Through kcrew
        for c = 1: 1: length(kcrew)
                        ...
                        if ERROR_SS(1) > 0.001 || ERROR_SS(2) > 0.001
                            ...
                            for f = 1: 1: length(SH.tau)
                                ...
                                if ERROR_SH(1) > 0.001 || ERROR_SH(2) > 0.001
                                    ...
                                    % Save Starship Data
                                    Data.Wpay(a).Ncrew(b).kcrew(c).hleo(d).v_sep(e).tau(i, f).SS = SSi;
        
                                    % Save Superheavy Data
                                    Data.Wpay(a).Ncrew(b).kcrew(c).hleo(d).v_sep(e).tau(i, f).SH = SHi;
